...
public:
    ...
    Sci::Position FindText(Document *doc, Sci::Position startPosition, Sci::Position endPosition, const char *regex,
                           bool caseSensitive, bool word, bool wordStart, int sciSearchFlags,
                           Sci::Position *lengthRet) override;
...
private:
    ...
    public:
        ...
        ~Match() override { setDocument(nullptr); }

        explicit Match(Document *document, Sci::Position position = -1, Sci::Position endPosition = -1) : _document(
                nullptr) { set(document, position, endPosition); }
    ...
    private:
        ...
        void NotifyModified(Document *modifiedDocument, DocModification mh, void * /*userData*/) override {
            if (modifiedDocument == _document) {
                if (mh.modificationType & (SC_PERFORMED_UNDO | SC_PERFORMED_REDO))
                    ...
                    if (mh.position == position() &&
                        position() == _endPositionForContinuationCheck) // Replace at last found position.
        ...
        void NotifyLexerChanged(Document * /*document*/, void * /*userData*/) override {}

        void NotifyErrorOccurred(Document * /*document*/, void * /*userData*/, int /*status*/) override {}

        Document *_document = nullptr;
        bool _documentModified = false;
        Sci::Position _position = -1, _endPosition = -1;
    ...
    public:
        explicit CharTPtr(const char *ptr) : _charPtr(ptr), _wcharPtr(nullptr) {}
    ...
    public:
        ...
        void compileRegex(const char *regex, int compileFlags);
...
Sci::Position BoostRegexSearch::FindText(Document *doc, Sci::Position startPosition, Sci::Position endPosition,
                                         const char *regexString,
                                         bool caseSensitive, bool /*word*/, bool /*wordStart*/, int sciSearchFlags,
                                         Sci::Position *lengthRet) {
    try {
        ...
        if (startPosition > endPosition
            || (startPosition == endPosition && _lastDirection <
                                               0))  // If we search in an empty region, suppose the direction is the same as last search (this is only important to verify if there can be an empty match in that empty region).
        {
        ...
        search._boostRegexFlags =
                ...
                | ((static_cast<unsigned int>(sciSearchFlags) & SCFIND_REGEXP_DOTMATCHESNL) ? regex_constants::match_default
        ...
        search._is_allowed_empty_at_start_position = search._is_allowed_empty &&
                                                     (allow_empty_at_start
                                                      || !_lastMatch.isContinuationSearch(doc, startPosition,
                                                                                          search._direction)
                                                      || (empty_match_style == SCFIND_REGEXP_EMPTYMATCH_ALL &&
                                                          !_lastMatch.isEmpty())    // If last match is empty and this is a continuation, then we would have same empty match at start position, if it was allowed.
                                                     );
...
template<class CharT, class CharacterIterator>
BoostRegexSearch::Match
BoostRegexSearch::EncodingDependent<CharT, CharacterIterator>::FindText(SearchParameters &search) {
...
template<class CharT, class CharacterIterator>
BoostRegexSearch::Match
BoostRegexSearch::EncodingDependent<CharT, CharacterIterator>::FindTextForward(SearchParameters &search) {
    ...
    do {
        ...
        found = !end_reached &&
                boost::regex_search(CharacterIterator(search._document, next_search_from_position, search._endPosition),
                                    endIterator, _match, _regex, search._boostRegexFlags);
        if (found) {
...
template<class CharT, class CharacterIterator>
BoostRegexSearch::Match
BoostRegexSearch::EncodingDependent<CharT, CharacterIterator>::FindTextBackward(SearchParameters &search) {
    ...
    for (;;) {
        ...
        if (endPosition > bestEnd && (endPosition < search._endPosition || position != endPosition ||
                                      is_allowed_empty_at_end_position)) // We are searching for the longest match which has the fathest end (but may not accept empty match at end position).
        {
            bestMatch = _match;
...
template<class CharT, class CharacterIterator>
void
BoostRegexSearch::EncodingDependent<CharT, CharacterIterator>::compileRegex(const char *regex, const int compileFlags) {
    if (_lastCompileFlags != compileFlags || _lastRegexString != regex) {
        _regex = Regex((const CharT *) CharTPtr(regex), static_cast<regex_constants::syntax_option_type>(compileFlags));
...
Sci::Position BoostRegexSearch::SearchParameters::nextCharacter(Sci::Position position) {
    if (_skip_windows_line_end_as_one_character && _document->CharAt(position) == '\r' &&
...
template<class CharT, class CharacterIterator>
char *BoostRegexSearch::EncodingDependent<CharT, CharacterIterator>::SubstituteByPosition(const char *text,
                                                                                          Sci::Position *length) {
...
wchar_t *BoostRegexSearch::utf8ToWchar(const char *utf8) {
    ...
    auto *w = new wchar_t[wcharSize + 1];
